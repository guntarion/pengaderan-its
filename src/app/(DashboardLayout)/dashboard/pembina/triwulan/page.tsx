...
/**
 * src/app/(DashboardLayout)/dashboard/pembina/triwulan/page.tsx
 * NAWASENA M14 — Pembina: List of reviews awaiting signature.
 */
...
const log = createLogger('m14/pembina/triwulan/list');
...
export default function PembinaTriwulanListPage() {
  ...
  useEffect(() => {
    async function fetchReviews() {
      setLoading(true);
      try {
        log.info('Fetching reviews awaiting Pembina signature');
      ...
      <div className="bg-gradient-to-r from-sky-500 to-blue-600 text-white py-6 px-4">
        <div className="container mx-auto max-w-3xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold">Review Menunggu Tanda Tangan</h1>
              <p className="text-sm text-white/70">Review triwulan untuk disetujui Pembina</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto max-w-3xl px-4 py-6 space-y-6">
        {urgentCount > 0 && (
          <div className="flex items-center gap-3 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-2xl p-4">
            ...
            <p className="text-sm text-red-700 dark:text-red-300">
              ...
              Wajib tinjauan tatap muka sebelum penandatanganan.
            </p>
          </div>
        )}
        ...
        <section>
          <h2 className="text-sm font-semibold text-gray-600 dark:text-gray-400 mb-3 uppercase tracking-wide">
            Menunggu Tanda Tangan ({reviews.length})
          </h2>
          ...
          {loading ? (
            ...
            <div className="text-center py-12 bg-white dark:bg-slate-800 rounded-2xl border border-sky-100 dark:border-sky-900">
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                Tidak ada review yang menunggu tanda tangan saat ini.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {reviews.map((r) => (
                <ReviewSummaryCard
                  ...
                  hrefBase="/dashboard/pembina/triwulan"
                  actionLabel="Tinjau & Tandatangani"
                />
              ))}
            </div>
          )}
