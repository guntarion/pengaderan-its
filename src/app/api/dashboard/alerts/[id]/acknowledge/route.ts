/**
 * src/app/api/dashboard/alerts/[id]/acknowledge/route.ts
 * POST /api/dashboard/alerts/:id/acknowledge
 * Mark a RedFlagAlert as ACKNOWLEDGED by the current user.
 */
...
export const POST = createApiHandler({
  ...
  handler: async (req, ctx) => {
    ...
    // Check role access: user's role must be in targetRoles
    ...
    if (!isAuthorized) {
      throw ForbiddenError('Anda tidak berwenang mengakui alert ini');
    }
    ...
    log.info('Alert acknowledged', { alertId: id, userId: user.id });
